...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    if dataset_name not in ['NUMBER','METTER','DOTA', 'ship', 'ICDAR2015', 'pascal', 'coco', 'DOTA_TOTAL', 'FDDB']:
        raise ValueError('dataSet name must be in pascal, coco spacenet and ship')

    if is_training:
        pattern = os.path.join('../data/tfrecord', cfgs.VERSION + '_' + dataset_name + '_train*')
    else:
        pattern = os.path.join('../data/tfrecord', cfgs.VERSION + '_' + dataset_name + '_test*')
